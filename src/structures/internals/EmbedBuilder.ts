...
export class EmbedBuilder {
  ...
  addField(name: string, value: string, inline: boolean = false) {
    if (!name) throw new Error('No embed name was set (EmbedBuilder#addField)');
    if (!value) throw new Error('No value was set (EmbedBuilder#addField)');
    if (this.fields!.length > 25) throw new Error('Reached the 25 field limit threshold');
